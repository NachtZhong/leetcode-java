...
public class PrettyPrintUtil {
    ...
    private static TreeNodeWidth recursiveCalculateWidth(TreeNode node, boolean isLeft, boolean isRoot) {
        ...
        TreeNodeWidth left = recursiveCalculateWidth(node.left, true, false);
        TreeNodeWidth right = recursiveCalculateWidth(node.right, false, false);
        int spaceL = 0;
        int spaceR = 0;
        int plen = String.valueOf(node.val).length();
        //将父节点的数值的宽度平摊到子节点上去
        ...
        TreeNodeWidth result = new TreeNodeWidth(node.val, Math.max(lnn && rnn ? spaceL - plen : (spaceL << 1) - plen, MIN_WIDTH), Math.max(lnn && rnn ? spaceR - plen : (spaceR << 1) - plen, MIN_WIDTH), left, right, isLeft);
